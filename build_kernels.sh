...
for kernel in $kernels; do
	...
	KCONFIG_NOTIMESTAMP=1 KBUILD_BUILD_TIMESTAMP='' KBUILD_BUILD_USER=chronos KBUILD_BUILD_HOST=localhost make -C "./kernels/$kernel" -j"$NTHREADS" LOCALVERSION="" O=out || { echo "Kernel build failed"; exit 1; }
